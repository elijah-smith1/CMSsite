...
import { useSite, usePageIndex } from '../../hooks/useSites';
...
export const SiteEditor: React.FC = () => {
  ...
  const { data: pageIndex, isLoading: indexLoading } = usePageIndex(siteId);
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);

  // Debug logging
  console.log('[CMS] SiteEditor - siteId:', siteId, 'pageId from URL:', pageId);

  if (siteLoading || indexLoading) {
  ...
  // The currently selected page ID comes ONLY from the URL
  // No fallback - if no page is selected, show the placeholder
  const currentPageId = pageId;

  // Use pageIndex for the page list (canonical source)
  const pageList = pageIndex?.pages || [];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      >
        ...
        {/* Pages List - Uses pageIndex as canonical source */}
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {pageList.length === 0 ? (
            ...
            <nav className="space-y-1">
              {pageList.map((pageEntry) => {
                // IMPORTANT: pageEntry.id is the Firestore document ID
                const isSelected = currentPageId === pageEntry.id;
                
                return (
                  <button
                    key={pageEntry.id}
                    onClick={() => {
                      // Navigate using the Firestore document ID
                      console.log('[CMS] Navigating to page:', pageEntry.id);
                      navigate(`/cms/sites/${siteId}/pages/${pageEntry.id}`);
                  ...
                  >
                    ...
                    <span className="truncate">{pageEntry.title}</span>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto">
          <Outlet context={{ siteId, site, pageIndex }} />
        </main>
      </div>
    </div>
  );
...
import { PageIndex } from '../../services/siteService';
